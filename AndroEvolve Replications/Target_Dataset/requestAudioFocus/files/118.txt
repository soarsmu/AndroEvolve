...
public class AudioFocusChangeManager {//
	...
	private static Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (flag)
				mode = 2;
			updateMode(AppContext.context(), mode);
		}

	};

	static {
		audioManger = (AudioManager) AppContext.context().getSystemService(
				Context.AUDIO_SERVICE);
	}

	public static void checkVolume() {//
		if (audioManger.getStreamVolume(AudioManager.STREAM_MUSIC) == 0) {
            //AppContext.showToastShort("请打开音量收听");
		}
	}
	...
	public static void updateToNormalMode() {//
		updateMode(AppContext.context(), AudioManager.MODE_NORMAL);
	}
	...
	public static boolean registerListener() {
		if (sensorListener == null)
			sensorListener = new ProximitySensorListener(AppContext.context(),
					handler);
		if (sensorListener == null) {
			return false;
		} else {
