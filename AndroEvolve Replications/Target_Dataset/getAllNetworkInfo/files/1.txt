...
public class NetworkUtil {
	...
	public static boolean printNetworkInfo(Context context) {
		...
		if (connectivity != null) {
			...
			log ="getActiveNetworkInfo: " + in + "\n";
			NetworkInfo[] info = connectivity.getAllNetworkInfo();
			if (info != null) {
				for (int i = 0; i < info.length; i++) {
					log += "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable() + "\n";
					log += "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected() + "\n";
					log += "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting() + "\n";
					log += "NetworkInfo[" + i + "]: " + info[i];
				}
			} else {
				log += "getAllNetworkInfo is null";
			}
		}
		Logger.i(log);
		return false;
