...
public class LocalPSMP extends PlaybackServiceMediaPlayer {
    ...
    private void resumeSync() {
        if (playerStatus == PlayerStatus.PAUSED || playerStatus == PlayerStatus.PREPARED) {
            ...
            int duration = AudioManager.AUDIOFOCUS_GAIN;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                ...
                AudioFocusRequest.Builder focusBuilder = new AudioFocusRequest.Builder(duration);
                focusBuilder.setAudioAttributes(audioAttributes);
                focusBuilder.setOnAudioFocusChangeListener(audioFocusChangeListener);
                focusBuilder.setAcceptsDelayedFocusGain(true);
                focusBuilder.setWillPauseWhenDucked(true);
                AudioFocusRequest audioFocusRequest = focusBuilder.build();
                focusGained = audioManager.requestAudioFocus(audioFocusRequest);
            } else {
                focusGained = audioManager.requestAudioFocus(audioFocusChangeListener, type, duration);
            }
